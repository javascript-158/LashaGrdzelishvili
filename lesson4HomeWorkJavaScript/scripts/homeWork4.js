...
function replaceCharacters(mail) {
  
  let atIndex = mail.indexOf('@');

  if (atIndex !== -1 && atIndex > 3) {
    
    let prefix = mail.substring(0, atIndex);

    
    let replacedString = mail.substring(0, 3) + mail.substring(3, atIndex).replace(/./g, '.') + mail.substring(atIndex);
...
console.log(replaceCharacters("Thisis@domain.com"));